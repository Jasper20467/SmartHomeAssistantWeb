...
class ChatGPTService:
    ...
    def _call_chatgpt_api(self, messages):
        """
        Sends a single request to the ChatGPT API and handles the response.
        """
        headers = {
        ...
        try:
            ...
            if 'choices' in result and len(result['choices']) > 0:
                ...
                try:
                    ...
                    # If not valid JSON, return as a standard text reply
                    self.logger.warning(f"ChatGPT response was not valid JSON. Content: {content}")
                    return {"action": "text_reply", "reply": content}
            else:
                self.logger.warning("ChatGPT API returned no choices.")
                return {"action": "text_reply", "reply": "抱歉，我無法產生回應。"}

        except requests.exceptions.HTTPError as http_err:
            # Specifically handle 429 Too Many Requests error
            if http_err.response.status_code == 429:
                self.logger.error("ChatGPT API rate limit exceeded (429). Please wait and try again later.")
                return {"action": "text_reply", "reply": "系統目前忙碌中，請稍後再試。"}
            else:
                self.logger.error(f"ChatGPT API HTTP error: {http_err}")
                return {"action": "text_reply", "reply": "處理您的訊息時發生錯誤，請再試一次。"}
        except Exception as e:
            self.logger.error(f"An unexpected error occurred during ChatGPT API call: {e}")
            return {"action": "text_reply", "reply": "處理您的訊息時發生錯誤，請再試一次。"}
    ...
    def process_message(self, user_message, user_id=None):
        ...
        gpt_response = {}
        try:
            # 1. Get context from backend and conversation history
            backend_context = self._get_backend_context(user_message)
            enhanced_prompt = self._enhance_prompt(user_message, backend_context)
            conversation_context = self._build_conversation_context(user_id)

            # 2. Construct the full message payload for the API
            system_prompt = (
                ...
                "Return: {\"action\": \"create_schedule\", \"parameters\": {\"title\": \"和朋友吃飯\", \"description\": \"這週六中午聚餐\", \"start_time\": \"2025-07-12T12:00:00Z\"}, \"reply\": \"已為您建立7月12日中午的聚餐排程。\"}\n"
                "2) User says: \"把今天晨跑改成一小時\"\n"
                "Return: {\"action\": \"get_schedule\", \"parameters\": {\"date\": \"2025-07-08\"}, \"reply\": \"請告訴我要修改哪一筆排程，請提供名稱或ID。\"}\n"
                "3) User says: \"更新ID 2，改成一小時\"\n"
                "Return: {\"action\": \"update_schedule\", \"parameters\": {\"id\": 2, \"end_time\": \"2025-07-08T07:00:00Z\"}, \"reply\": \"已更新ID 2的排程，時間已延長至一小時。\"}\n"
                "4) User says: \"取消這週六的活動\"\n"
                "Return: {\"action\": \"get_schedule\", \"parameters\": {\"date\": \"2025-07-12\"}, \"reply\": \"我找到7月12日的排程，請告訴我要取消哪一筆活動，請提供名稱或ID。\"}\n"
                "5) User says: \"取消ID 3\"\n"
                "Return: {\"action\": \"delete_schedule\", \"parameters\": {\"id\": 3}, \"reply\": \"已為您取消ID 3的排程。\"}\n"
                "6) User says: \"目前有哪些消耗品？\"\n"
                "Return: {\"action\": \"get_consumable\", \"parameters\": {}, \"reply\": \"正在查詢目前的消耗品...\"}\n"
                "7) User says: \"你好！\"\n"
                "Return: {\"action\": \"text_reply\", \"reply\": \"你好！有什麼我可以幫忙的？\"}\n"
            )
            ...
            # 3. Call the API once and get the response
            gpt_response = self._call_chatgpt_api(messages)

        except Exception as e:
            # Catch any other unexpected errors during prompt preparation
            self.logger.error(f"Error preparing data for ChatGPT API: {e}")
            gpt_response = {
                "action": "text_reply",
                "reply": "處理您的訊息時發生內部錯誤，請再試一次。"
            }
        finally:
            # 4. Always add the interaction to history, even if an error occurred
            self._add_to_conversation_history(user_id, user_message, gpt_response)
